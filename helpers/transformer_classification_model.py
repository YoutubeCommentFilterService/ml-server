...
class TransformerClassificationModel:
    ...
    def load(self, tokenizer: Union[PreTrainedTokenizer, PreTrainedTokenizerFast] = None):
        try:
            if self.model:
                return
    ...
    def reload(self, tokenizer: Union[PreTrainedTokenizer, PreTrainedTokenizerFast] = None):
        self.unload()
    ...
    def predict(self, texts: Union[str, List[str]]) -> List[Tuple[str, float]]:
        ...
        def predict_short_texts(texts: List[Union[int, str]]):
            ...
            tokens = self.tokenizer(list(batch_texts),
                                    return_tensors="pt",
                                    padding=True,
                                    truncation=True,
                                    padding_side="right",
        ...
        def predict_long_texts(inputs, local_indicies, grouped_data):
            def pad_tensor(tensors: List[DefaultDict[str, List[torch.Tensor]]]) -> Dict[str, torch.Tensor]:
                ...
                return {
                    key: torch.stack([
                            F.pad(tensor, (0, max_length - len(tensor)), mode='constant', value=self.tokenizer.pad_token_id if key == 'input_ids' else 0) for d_dict in tensors for tensor in d_dict[key]
                        ]) for key in keys
                }
        ...
        for loop_idx, value in enumerate(long_texts):
            idx, text = value
            tensors, tensor_lengths = self._calculate_token_length(text)
            ...
            if local_sum > batch_size:
                predict_long_texts(inputs, local_indicies, grouped_data)

                inputs = []
                local_indicies = []
                local_sum = 0

            if loop_idx % (batch_size // 2) == 0:
                torch.cuda.empty_cache()
    ...
    def _calculate_token_length(self, text: str):
        def process_token(tokens, start_index, end_index):
        ...
        for sentence in merged_sentences:
            ...
            while cur_token_idx < token_length:
                # 두 값의 차이가 {{ threshold }} 초과라면 현재 문장이 유의미한 문장임을 암시
                # 그것이 아니라면 유의미하지 않음 -> 유의미한 전 토큰을 가져와서 토큰화
                if (token_length - cur_token_idx) > threshold:
                    local_result, batched_lengths = process_token(
                        tokens, 
                        cur_token_idx,
